...
void Plot_JpsiV2_a1_Final(){
  ...
  for(int iCat = 0; iCat < 2; iCat++){
    ...
    const int nptbins   = 2; 
    int    cts[ncentbins]  = {0, 10, 20, 30, 60};
    ...
    double gErrP0010[4] = {0.05, 0.05, 0.05, 0.05}; // error of etHFp for 0 - 10 %
    double gErrP1020[4] = {0.05, 0.05, 0.05, 0.05}; // error of etHFp for 10 - 20 %
    double gErrP2030[4] = {0.05, 0.05, 0.05, 0.05}; // error of etHFp for 20 - 30 %
    double gErrP3060[4] = {0.05, 0.05, 0.05, 0.05}; // error of etHFp for 30 - 60 %

    double gErrM0010[4] = {0.05, 0.05, 0.05, 0.05}; // error of etHFm for 0 - 10 %
    double gErrM1020[4] = {0.05, 0.05, 0.05, 0.05}; // error of etHFm for 10 - 20 %
    double gErrM2030[4] = {0.05, 0.05, 0.05, 0.05}; // error of etHFm for 20 - 30 %
    double gErrM3060[4] = {0.05, 0.05, 0.05, 0.05}; // error of etHFm for 30 - 60 %
    ...
    for(int choseSignal = 0; choseSignal<1; choseSignal++){
      ...
      for(int mcent = centrality_start; mcent < centrality_end; mcent++){
        ...
        for(int iy = y_start; iy < y_end; iy++){
          ...
          for(int jpt = pt_start; jpt < pt_end; jpt++){
            ...
            if(mcent == 0 && choseSignal == 0) pgTemp1 = new TGraphErrors(4, b, c, e, gErrP0010);
            if(mcent == 0 && choseSignal == 1) pgTemp1 = new TGraphErrors(4, b, c, e, gErrM0010);
            if(mcent == 1 && choseSignal == 0) pgTemp1 = new TGraphErrors(4, b, c, e, gErrP1020);
            if(mcent == 1 && choseSignal == 1) pgTemp1 = new TGraphErrors(4, b, c, e, gErrM1020);
            if(mcent == 2 && choseSignal == 0) pgTemp1 = new TGraphErrors(4, b, c, e, gErrP2030);
            if(mcent == 2 && choseSignal == 1) pgTemp1 = new TGraphErrors(4, b, c, e, gErrM2030);
            if(mcent == 3 && choseSignal == 0) pgTemp1 = new TGraphErrors(4, b, c, e, gErrP3060);
            if(mcent == 3 && choseSignal == 1) pgTemp1 = new TGraphErrors(4, b, c, e, gErrM3060);
        ...
        for(int ky = y_start; ky < y_end; ky++)
        {
          ...
          for(int lpt = pt_start; lpt < pt_end; lpt++)
          {
            ...
            if(ind==0 )
            {
              ...
              lt1->DrawLatex(0.24,0.23,Form("v_{2} = %.4f #pm %.4f (#chi^{2} / ndf = %.3f / %.0f)", pr[mcent][ky][lpt], ep[mcent][ky][lpt], chi[mcent][ky][lpt], ndf[mcent][ky][lpt]));
            ...
            {
              lt1->SetTextSize(0.05);
              lt1->DrawLatex(0.05,0.23,Form("v_{2} = %.4f #pm %.4f (#chi^{2} / ndf = %.3f / %.0f)", pr[mcent][ky][lpt], ep[mcent][ky][lpt], chi[mcent][ky][lpt], ndf[mcent][ky][lpt]));
      ...
      {
        if(iCat == 0){
          pc1->SaveAs(Form("plots/etHFm/%s_%s_Jpsi_%d_%d_a1_Final_Nominal_0302.png",chosenSignal,nameoutfile,pt_start,pt_end));
          pc1->SaveAs(Form("plots/etHFm/%s_%s_Jpsi_%d_%d_a1_Final_Nominal_0302.pdf",chosenSignal,nameoutfile,pt_start,pt_end));
        }
        if(iCat == 1){
          pc1->SaveAs(Form("plots/etHFp/%s_%s_Jpsi_%d_%d_a1_Final_Nominal_0302.png",chosenSignal,nameoutfile,pt_start,pt_end));
          pc1->SaveAs(Form("plots/etHFp/%s_%s_Jpsi_%d_%d_a1_Final_Nominal_0302.pdf",chosenSignal,nameoutfile,pt_start,pt_end));
        }
        if(iCat == 2){
          pc1->SaveAs(Form("plots/etHF/%s_%s_Jpsi_%d_%d_a1_Final_Nominal_0302.png",chosenSignal,nameoutfile,pt_start,pt_end));
          pc1->SaveAs(Form("plots/etHF/%s_%s_Jpsi_%d_%d_a1_Final_Nominal_0302.pdf",chosenSignal,nameoutfile,pt_start,pt_end));
        }
      ...
      cout<<"|  v2  |"<<"  "<<pr[0][0][0]<<"  |  "<<pr[1][0][0]<<"  |  "<<pr[2][0][0]<<"  |  "<<pr[3][0][0]<<"  |"<<endl;
      cout<<"|  err  |"<<"  "<<ep[0][0][0]<<"  |  "<<ep[1][0][0]<<"  |  "<<ep[2][0][0]<<"  |  "<<ep[3][0][0]<<"  |"<<endl;
      cout<<""<<endl;
      //leg1->Draw("same");
      ...
      if(iCat == 0){
        c2->SaveAs(Form("plots/etHFm/%s_jpsi_%s_v2_pt_a1_Final_Nominal_0302.png",chosenSignal,nameoutfile));
        c2->SaveAs(Form("plots/etHFm/%s_jpsi_%s_v2_pt_a1_Final_Nominal_0302.pdf",chosenSignal,nameoutfile));
      }
      if(iCat == 1){
        c2->SaveAs(Form("plots/etHFp/%s_jpsi_%s_v2_pt_a1_Final_Nominal_0302.png",chosenSignal,nameoutfile));
        c2->SaveAs(Form("plots/etHFp/%s_jpsi_%s_v2_pt_a1_Final_Nominal_0302.pdf",chosenSignal,nameoutfile));
      }
      if(iCat == 2){
        c2->SaveAs(Form("plots/etHF/%s_jpsi_%s_v2_pt_a1_Final_Nominal_0302.png",chosenSignal,nameoutfile));
        c2->SaveAs(Form("plots/etHF/%s_jpsi_%s_v2_pt_a1_Final_Nominal_0302.pdf",chosenSignal,nameoutfile));
      }
    }
  }
