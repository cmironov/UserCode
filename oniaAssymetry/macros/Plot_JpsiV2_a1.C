...
void Plot_JpsiV2_a1(){
  ...
  const char* nameoutfile[2] = {"","etHF"};

  const char* eventPlane[2] = {"","EP: etHF"};
  ...
  lt1->SetTextSize(0.06);
  lt1->DrawLatex(0.05,0.82,Form("%s",eventPlane[1]));
  //_______
  if(bSavePlots)
  {
    pc1->SaveAs(Form("plots/etHF/%s_%s_Jpsi_%d_%d_150mub_a1.png",chosenSignal,nameoutfile[1],pt_start,pt_end));
    pc1->SaveAs(Form("plots/etHF/%s_%s_Jpsi_%d_%d_150mub_a1.pdf",chosenSignal,nameoutfile[1],pt_start,pt_end));
  ...
  for(int ic = centrality_end-1; ic >= centrality_start; ic--)
  {
    ...
    int stl = 20;
    /*
    if(ic == 0) {stl = 20;sz = 1.8; scol = 602;}
    if(ic == 1) {stl = 21;sz = 1.6; scol = 634;}
    if(ic == 2) {stl = 29;sz = 2.2; scol = 418;}
    if(ic == 3) {stl = 33;sz = 2.2; scol = 862;}
    */
  ...
  c2->SaveAs(Form("plots/etHF/%s_jpsi_%s_v2_pt_a1.png",chosenSignal,nameoutfile[1]));
  c2->SaveAs(Form("plots/etHF/%s_jpsi_%s_v2_pt_a1.pdf",chosenSignal,nameoutfile[1]));
