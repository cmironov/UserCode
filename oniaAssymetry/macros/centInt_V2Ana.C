...
//   TFile *f1 = new TFile("separate150_fit1_newBinsNormRight/ep21_2sig_pNp/xopt21_defaults/summary/saved_histo.root");
//   const char* nameoutfile[2] = {"",
// 				"ep21_2sig_xopt2_defaults_centInt"
//   };

//   const char* eventPlane[2] = {"",
// 			       //"EventPlane: etHFp & etHFm"
// 			       "EP: etHF"
//   };
...
//   };
 ...
 TFile *f1 = new TFile("inclusive150_jpsi/InclusiveYield/ep22/xopt21_bit2/summary/saved_histo.root");
	  ...
	  for(int lc = centrality_start; lc < centrality_end; lc++)
	      ...
	      if(!g[lc][ky][mpt]) 
		{
	      ...
	      if(ind==0 )
 		{
		  ...
		  lt1->DrawLatex(0.23,0.3,Form("v_{2}: %.4f #pm %.4f (#chi^{2} / ndof: %.3f / %.0f)", pr[lc][ky][mpt], ep[lc][ky][mpt], chi[lc][ky][mpt], ndf[lc][ky][mpt]));
		  lt1->DrawLatex(0.25,0.2,Form("%.1f < p_{T} <%.1f", vpts1, vpts2)); 
		}
 	      else
 		{
		  lt1->SetTextSize(0.05);
 		  lt1->DrawLatex(0.05,0.3,Form("v_{2}: %.4f #pm %.4f (#chi^{2} / ndof: %.3f / %.0f)", pr[lc][ky][mpt], ep[lc][ky][mpt], chi[lc][ky][mpt], ndf[lc][ky][mpt]));
		  lt1->DrawLatex(0.1,0.2,Form("%.1f < p_{T} <%.1f", vpts1, vpts2)); 
 		}
	    }//centrality bin loop
	}//rapidity bin loop
        ind++;
  ...
  if(bSavePlots)
    {
      pc1->SaveAs(Form("%s_%s_Jpsi_fit1_%d_%d_150mub.png",chosenSignal,nameoutfile[1],pt_start,pt_end));
      pc1->SaveAs(Form("%s_%s_Jpsi_fit1_%d_%d_150mub.pdf",chosenSignal,nameoutfile[1],pt_start,pt_end));
  ...
  TH1F *hPad2 = new TH1F("hPad2",";p_{T};Uncorrected v2;",100,0,40);
  ...
  tex6->SetLineWidth(2);
  tex6->Draw();
  lt1->DrawLatex(0.8,0.3,Form("%s",legend[choseSignal]));  // what signal is
  ...
  for(int ic = pt_start; ic < pt_end; ic++)
    {
      ...
      gPtBarr[ic]->SetMarkerColor(ic+1);
       
      gPtBarr[ic]->Draw("pz");
      vpts1 = pts[ic]; vpts2 = pts[ic+1];
      leg1->AddEntry(gPtBarr[ic],Form("%.1f < p_{T} <%.1f", vpts1, vpts2),"P");
    
      cout<<" producing the v2 plots in the Canvas : "<<ic<<" "<<endl;
      c2->Update();
    }
  leg1->Draw("same");
  lt1->DrawLatex(0.25,0.4,Form("|y| < %.1f",vraps2));       // rapidity
  lt1->DrawLatex(0.25,0.3,Form("10-60 %%")); 
