...
void Plot_JpsiV2_a3()
{
  ...
  for(int mcent = centrality_start; mcent < centrality_end; mcent++)
  {
    ...
    TCanvas *pc1 = new TCanvas("pc1",Form("pcV2_%d-%d",vcts1,vcts2),0,0,820,750);
    ...
    pp->GetXaxis()->SetTitleOffset(1.8);
    ...
    pp->GetYaxis()->SetTitleOffset(2.0);
    ...
    for(int ky = 0; ky < 4; ky+=2)
    {
      ...
      for(int lpt = pt_start; lpt < pt_end; lpt++)
      {
        ...
        if(ind == 1) 
        {
          ...
          lt1->DrawLatex(0.25,0.2,Form("%s",eventPlane[1]));
        }
        if(ind == 2) 
        {
          lt1->SetTextSize(0.06);
    ...
    TLatex *tex2 = new TLatex(0.25,0.84,"PbPb  #sqrt{s_{NN}} = 2.76 TeV");
    tex2->SetNDC();
    ...
    TLatex *tex3 = new TLatex(0.25,0.75,"L_{int} = 150 #mub^{-1}");
    tex3->SetNDC();
    ...
    if(bSavePlots)
    {
      pc1->SaveAs(Form("plots/etHF/%s_%s_Jpsi_%d_%d_150mub_a3.png",chosenSignal,nameoutfile[1],vcts1,vcts2));
      pc1->SaveAs(Form("plots/etHF/%s_%s_Jpsi_%d_%d_150mub_a3.pdf",chosenSignal,nameoutfile[1],vcts1,vcts2));
  ...
  TLegend *leg1 = new TLegend(0.1812081,0.1958042,0.4395973,0.3024476);
  leg1->SetFillColor(0);
  ...
  if(bSavePlots)
  {
    c2->SaveAs(Form("plots/etHF/%s_jpsi_%s_v2_y_a3.png",chosenSignal,nameoutfile[1]));
    c2->SaveAs(Form("plots/etHF/%s_jpsi_%s_v2_y_a3.pdf",chosenSignal,nameoutfile[1]));
  }
...
void GetV2(TGraphErrors *a, double *b)
{
  ...
  b[0] = v2Fit1->GetParameter(1);
  b[1] = v2Fit1->GetParError(1);
