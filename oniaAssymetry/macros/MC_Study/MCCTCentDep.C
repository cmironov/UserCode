...
void MCCTCentDep()   
{
  for(int iSpec = 0; iSpec < 3; iSpec++){
    ...
    for(int iCat = 0; iCat < 4; iCat++){
      ...
      if(iCat == 0) sprintf(OutTextFile,"MCCT_PrJpsi_Cent_0010_dPhi_%s.tex", cCd);
      if(iCat == 1) sprintf(OutTextFile,"MCCT_PrJpsi_Cent_1020_dPhi_%s.tex", cCd);
      if(iCat == 2) sprintf(OutTextFile,"MCCT_PrJpsi_Cent_2030_dPhi_%s.tex", cCd);
      if(iCat == 3) sprintf(OutTextFile,"MCCT_PrJpsi_Cent_3060_dPhi_%s.tex", cCd);
      ...
      if(Prompt==1){
        ...
        //2.06586e-07 2.03431e-07 4.44677e-08 9.80087e-09 4.7002e-09 2.57384e-09
        ...
        //scale[6]={2.35829e-07, 1.99854e-07, 4.48263e-08, 1.01144e-08, 4.89604e-09, 2.62102e-09}; // JpsiGenPsi
        scale[0]=2.35829e-07;
        scale[1]=1.99854e-07;
        scale[2]=4.48263e-08;
        scale[3]=1.01144e-08;
      ...
      for(int ifile =0; ifile<=5; ifile++){
        ...
        for(int i=0; i< nGenEntries; i++)  {        
          ...
          for (Int_t ih = 0; ih < Nptbin; ih++) {
              if(iCat == 0)  {
                if( (gbin >= 0 && gbin < 4) && (GenPosIn==1 && GenNegIn==1) && 
              ...
              if(iCat == 1)  {
                if( (gbin >= 4 && gbin < 8) && (GenPosIn==1 && GenNegIn==1) && 
                  (GenJpsiPt >= 6.5 && GenJpsiPt < 40.0) && (TMath::Abs(GenJpsiRap)<2.4 && TMath::Abs(GenJpsiRap) >= 0.0 ) && 
                  (TMath::Abs(GenJpsiPsi)>=pt_bound[ih] && TMath::Abs(GenJpsiPsi)<pt_bound[ih+1])){diMuonsInvMass_GenA[ifile][ih]->Fill(GenJpsiMass,GenWeight);}
              }
              if(iCat == 2)  {
                if( (gbin >= 8 && gbin < 12) && (GenPosIn==1 && GenNegIn==1) && 
                  (GenJpsiPt >= 6.5 && GenJpsiPt < 40.0) && (TMath::Abs(GenJpsiRap)<2.4 && TMath::Abs(GenJpsiRap) >= 0.0 ) && 
                  (TMath::Abs(GenJpsiPsi)>=pt_bound[ih] && TMath::Abs(GenJpsiPsi)<pt_bound[ih+1])){diMuonsInvMass_GenA[ifile][ih]->Fill(GenJpsiMass,GenWeight);}
              }
              if(iCat == 3)  {
                if( (gbin >= 12 && gbin < 24) && (GenPosIn==1 && GenNegIn==1) && 
                  (GenJpsiPt >= 6.5 && GenJpsiPt < 40.0) && (TMath::Abs(GenJpsiRap)<2.4 && TMath::Abs(GenJpsiRap) >= 0.0 ) && 
                  (TMath::Abs(GenJpsiPsi)>=pt_bound[ih] && TMath::Abs(GenJpsiPsi)<pt_bound[ih+1])){diMuonsInvMass_GenA[ifile][ih]->Fill(GenJpsiMass,GenWeight);}
              }
          }
      ...
      for (Int_t ih = 0; ih < Nptbin; ih++) {
        ...
        sprintf(PlotName,"plots/DiMuonMass_Cent_%d_dPhiBin_%d_%s.png",iCat, ih, cCd);
        sprintf(PlotName1,"plots/DiMuonMass_Cent_%d_dPhiBin_%d_%s.pdf",iCat, ih, cCd);
        ...
        sprintf(GPlotName,"plots/DiMuonGenMass_Cent_%d_dPhiBin_%d_%s.png",iCat,ih,cCd);
        sprintf(GPlotName1,"plots/DiMuonGenMass_Cent_%d_dPhiBin_%d_%s.pdf",iCat,ih,cCd);
      ...
      TFile *outfile;
      char tmp_output[512];
      if(iCat == 0) sprintf(tmp_output,"MCCT_PrJpsi_Cent_0010_dPhi_%s.root",cCd);
      if(iCat == 1) sprintf(tmp_output,"MCCT_PrJpsi_Cent_1020_dPhi_%s.root",cCd);
      if(iCat == 2) sprintf(tmp_output,"MCCT_PrJpsi_Cent_2030_dPhi_%s.root",cCd);
      if(iCat == 3) sprintf(tmp_output,"MCCT_PrJpsi_Cent_3060_dPhi_%s.root",cCd);
      ...
      if(iCat == 0){
        Eff_Jpsi->SetName("Eff_Jpsi_Cent_0010");
        Jpsi_Gen->SetName("Gen_Jpsi_Cent_0010");
        Jpsi_Reco->SetName("Reco_Jpsi_Cent_0010");
      ...
      if(iCat == 1){
        Eff_Jpsi->SetName("Eff_Jpsi_Cent_1020");
        Jpsi_Gen->SetName("Gen_Jpsi_Cent_1020");
        Jpsi_Reco->SetName("Reco_Jpsi_Cent_1020");
      ...
      if(iCat == 2){
        Eff_Jpsi->SetName("Eff_Jpsi_Cent_2030");
        Jpsi_Gen->SetName("Gen_Jpsi_Cent_2030");
        Jpsi_Reco->SetName("Reco_Jpsi_Cent_2030");
      ...
      if(iCat == 3){
        Eff_Jpsi->SetName("Eff_Jpsi_Cent_3060");
        Jpsi_Gen->SetName("Gen_Jpsi_Cent_3060");
        Jpsi_Reco->SetName("Reco_Jpsi_Cent_3060");
      ...
      double effErr = stline->GetParError(0);
      cout<<" efficiency : "<<effNo<<endl;
      char tmp[512], tmp1[512];
      sprintf(tmp,"PythiaEvtGen + HydjetBass : %.2f #pm %.2f %%", effNo*100, effErr*100);
      legend_GP->AddEntry(Eff_Jpsi,tmp, "PL");
      legend_GP->Draw("Same");

      if(iCat == 0) sprintf(tmp1,"0 - 10 %%");
      if(iCat == 1) sprintf(tmp1,"10 - 20 %%");
      if(iCat == 2) sprintf(tmp1,"20 - 30 %%");
      if(iCat == 3) sprintf(tmp1,"30 - 60 %%");
      lt1->DrawLatex(0.21,0.91,tmp1);

      //Eff_Jpsi->Write();

      char tmp_pdf[512], tmp_png[512];
      if(iCat == 0) {sprintf(tmp_pdf,"plots/Eff_Jpsi_Cent_0010_dPhi_%s_%s.pdf",cCd);sprintf(tmp_png,"plots/Eff_Jpsi_Cent_0010_dPhi_%s_%s.png",cCd);}
      if(iCat == 1) {sprintf(tmp_pdf,"plots/Eff_Jpsi_Cent_1020_dPhi_%s_%s.pdf",cCd);sprintf(tmp_png,"plots/Eff_Jpsi_Cent_1020_dPhi_%s_%s.png",cCd);}
      if(iCat == 2) {sprintf(tmp_pdf,"plots/Eff_Jpsi_Cent_2030_dPhi_%s_%s.pdf",cCd);sprintf(tmp_png,"plots/Eff_Jpsi_Cent_2030_dPhi_%s_%s.png",cCd);}
      if(iCat == 3) {sprintf(tmp_pdf,"plots/Eff_Jpsi_Cent_3060_dPhi_%s_%s.pdf",cCd);sprintf(tmp_png,"plots/Eff_Jpsi_Cent_3060_dPhi_%s_%s.png",cCd);}
      gPad->Print(tmp_pdf);
      gPad->Print(tmp_png);
...
void DoEffCor(TFile* a, int b, double c, double d, double e, double *f){
  ...
  double bpt[6] = {3.0, 4.5, 6.5, 8.0, 10.0, 40.0}; // 3.0, 4.5, 6.5, 8.0, 10.0, 40.0
  int ict = 0;
  int ipt = 0;
  if(c >= 4 && c < 8) ict = 0; 
  if(c >= 8 && c < 12) ict = 1; 
  ...
  sprintf(tmp,"eff1D_Cent_%1.f_%1.f_Pt_%0.1f_%0.1f_%s",bct[ict],bct[ict+1],bpt[ipt],bpt[ipt+1], icCd);
