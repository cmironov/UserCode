...
void MCCTPtDep()   
{
  for(int iSpec = 0; iSpec < 3; iSpec++){
    ...
    for(int iCat = 0; iCat < 4; iCat++){
      ...
      if(iCat == 0) sprintf(OutTextFile,"MCCT_PrJpsi_Pts_030065_dPhi_%s.tex", cCd);
      if(iCat == 1) sprintf(OutTextFile,"MCCT_PrJpsi_Pts_065080_dPhi_%s.tex", cCd);
      if(iCat == 2) sprintf(OutTextFile,"MCCT_PrJpsi_Pts_080100_dPhi_%s.tex", cCd);
      if(iCat == 3) sprintf(OutTextFile,"MCCT_PrJpsi_Pts_100400_dPhi_%s.tex", cCd);
      //if(iSpec==5 && iCat == 4) sprintf(OutTextFile,"MCCT_PrJpsi_Pts_130400_dPhi_default.tex");
      ...
      for (int ifile = 0; ifile <= 6; ifile++) {
        for (Int_t ih = 0; ih < Nptbin; ih++) {
          sprintf(nameGen[ifile],"DiMuonMassGen_pt_%d_%d_%d",ih,ifile,iCat);
          sprintf(nameRec[ifile],"DiMuonMassRec_pt_%d_%d_%d",ih,ifile,iCat);

          sprintf(nameGenPt[ifile],"DiMuonPtGen_pt_%d_%d_%d",ih,ifile,iCat);
          sprintf(nameRecPt[ifile],"DiMuonPtRec_pt_%d_%d_%d",ih,ifile,iCat);
      ...
      for(int ifile =0; ifile<=5; ifile++){
        ...
        for(int i=0; i< nGenEntries; i++)  {        
          ...
          for (Int_t ih = 0; ih < Nptbin; ih++) {
            ...
            //adding pT of all pt bins to see diss is cont
              ...
              if(iCat == 0)  {
                if(  (gbin >= 4 && gbin < 24) && (GenPosIn==1 && GenNegIn==1) && (GenJpsiPt >= 3.0 && GenJpsiPt < 6.5) &&  
                    (TMath::Abs(GenJpsiRap)<2.4 && TMath::Abs(GenJpsiRap) >= 0.0 ) && 
                    (TMath::Abs(GenJpsiPsi)>=pt_bound[ih] && TMath::Abs(GenJpsiPsi)<pt_bound[ih+1])){diMuonsInvMass_GenA[ifile][ih]->Fill(GenJpsiMass,GenWeight);}
              }
              if(iCat == 1)  {
                if(  (gbin >= 4 && gbin < 24) && (GenPosIn==1 && GenNegIn==1) && (GenJpsiPt >= 6.5 && GenJpsiPt < 8.0) &&  
                    (TMath::Abs(GenJpsiRap)<2.4 && TMath::Abs(GenJpsiRap) >= 0.0 ) && 
                    (TMath::Abs(GenJpsiPsi)>=pt_bound[ih] && TMath::Abs(GenJpsiPsi)<pt_bound[ih+1])){diMuonsInvMass_GenA[ifile][ih]->Fill(GenJpsiMass,GenWeight);}
              }
              if(iCat == 2)  {
                if(  (gbin >= 4 && gbin < 24) && (GenPosIn==1 && GenNegIn==1) && (GenJpsiPt >= 8.0 && GenJpsiPt < 10.0) &&  
                    (TMath::Abs(GenJpsiRap)<2.4 && TMath::Abs(GenJpsiRap) >= 0.0 ) && 
                    (TMath::Abs(GenJpsiPsi)>=pt_bound[ih] && TMath::Abs(GenJpsiPsi)<pt_bound[ih+1])){diMuonsInvMass_GenA[ifile][ih]->Fill(GenJpsiMass,GenWeight);}
              }
              if(iCat == 3)  {
                if(  (gbin >= 4 && gbin < 24) && (GenPosIn==1 && GenNegIn==1) && (GenJpsiPt >= 10.0 && GenJpsiPt < 40.0) &&  
                    (TMath::Abs(GenJpsiRap)<2.4 && TMath::Abs(GenJpsiRap) >= 0.0 ) && 
                    (TMath::Abs(GenJpsiPsi)>=pt_bound[ih] && TMath::Abs(GenJpsiPsi)<pt_bound[ih+1])){diMuonsInvMass_GenA[ifile][ih]->Fill(GenJpsiMass,GenWeight);}
              }
          }
        ...
        for(int i=0; i<nRecEntries; i++)  {     
          ...
          double JpsidPhi = 0.0;
          //if(JpsiEta > 0) JpsidPhi = TMath::Abs(JpsiGenPsi);
          //if(!(JpsiEta > 0)) JpsidPhi = TMath::Abs(JpsiPsi[22]);
            ...
            if((JpsiCharge == 0) && (JpsiVprob > 0.01)) {     
              ...
              if(iCat == 0) {
                if((AllCut==1) && (rbin >= 4 && rbin < 24) && (JpsiPt>=3.0 && JpsiPt<6.5) && 
                    (TMath::Abs(JpsiRap) < 2.4 && TMath::Abs(JpsiRap) >= 0.0) && 
              ...
              if(iCat == 1) {
                if((AllCut==1) && (rbin >= 4 && rbin < 24) && (JpsiPt>=6.5 && JpsiPt<8.0) && 
                    (TMath::Abs(JpsiRap) < 2.4 && TMath::Abs(JpsiRap) >= 0.0) && 
                    (TMath::Abs(JpsiGenPsi) > pt_bound[ih] && TMath::Abs(JpsiGenPsi) <=pt_bound[ih+1])){
                  DoEffCor(eff_input, iCd, rbin, JpsiPt, JpsiRap, eff_cor);
                  //DoEffCor(eff_input, tmp_input_histo, TMath::Abs(JpsiGenPsi), JpsiPt, TMath::Abs(JpsiRap), eff_cor);
              ...
              if(iCat == 2) {
                if((AllCut==1) && (rbin >= 4 && rbin < 24) && (JpsiPt>=8.0 && JpsiPt<10.0) && 
                    (TMath::Abs(JpsiRap) < 2.4 && TMath::Abs(JpsiRap) >= 0.0) && 
                    (TMath::Abs(JpsiGenPsi) > pt_bound[ih] && TMath::Abs(JpsiGenPsi) <=pt_bound[ih+1])){
                  DoEffCor(eff_input, iCd, rbin, JpsiPt, JpsiRap, eff_cor);
                  //DoEffCor(eff_input, tmp_input_histo, TMath::Abs(JpsiGenPsi), JpsiPt, TMath::Abs(JpsiRap), eff_cor);
              ...
              if(iCat == 3) {
                if((AllCut==1) && (rbin >= 4 && rbin < 24) && (JpsiPt>=10.0 && JpsiPt<40.0) &&
                  (TMath::Abs(JpsiRap) < 2.4 && TMath::Abs(JpsiRap) >= 0.0) && 
                  (TMath::Abs(JpsiGenPsi) > pt_bound[ih] && TMath::Abs(JpsiGenPsi) <=pt_bound[ih+1])){
                DoEffCor(eff_input, iCd, rbin, JpsiPt, JpsiRap, eff_cor);
                //DoEffCor(eff_input, tmp_input_histo, TMath::Abs(JpsiGenPsi), JpsiPt, TMath::Abs(JpsiRap), eff_cor);
      ...
      for (Int_t ih = 0; ih < Nptbin; ih++) {
        ...
        sprintf(PlotName,"plots/DiMuonMass_Pt_%d_PsiBin_%d_%s.png",iCat, ih, cCd);
        sprintf(PlotName1,"plots/DiMuonMass_Pts_%d_PsiBin_%d_%s.pdf",iCat, ih, cCd);
        ...
        sprintf(GPlotName,"plots/DiMuonGenMass_Pt_%d_PsiBin_%d_%s.png",iCat,ih,cCd);
        sprintf(GPlotName1,"plots/DiMuonGenMass_Pt_%d_PsiBin_%d_%s.pdf",iCat,ih,cCd);
      ...
      TFile *outfile;
      char tmp_output[512];
      if(iCat == 0) sprintf(tmp_output,"MCCT_PrJpsi_Pts_030065_dPhi_%s.root",cCd);
      if(iCat == 1) sprintf(tmp_output,"MCCT_PrJpsi_Pts_065080_dPhi_%s.root",cCd);
      if(iCat == 2) sprintf(tmp_output,"MCCT_PrJpsi_Pts_080100_dPhi_%s.root",cCd);
      if(iCat == 3) sprintf(tmp_output,"MCCT_PrJpsi_Pts_100400_dPhi_%s.root",cCd);
      ...
      if(iCat == 0){
        Eff_Jpsi->SetName("Eff_Jpsi_Pts_030065");
        Jpsi_Gen->SetName("Gen_Jpsi_Pts_030065");
        Jpsi_Reco->SetName("Reco_Jpsi_Pts_030065");
        hJpsi_Gen->SetName("hGen_Jpsi_Pts_030065");
        hJpsi_Reco->SetName("hReco_Jpsi_Pts_030065");
        Jpsi_Reco_Norm->SetName("nReco_Jpsi_Pts_030065");
        Jpsi_Gen_Norm->SetName("nGen_Jpsi_Pts_030065");
      }
      if(iCat == 1){
        Eff_Jpsi->SetName("Eff_Jpsi_Pts_065080");
        Jpsi_Gen->SetName("Gen_Jpsi_Pts_065080");
        Jpsi_Reco->SetName("Reco_Jpsi_Pts_065080");
        hJpsi_Gen->SetName("hGen_Jpsi_Pts_065080");
        hJpsi_Reco->SetName("hReco_Jpsi_Pts_065080");
        Jpsi_Reco_Norm->SetName("nReco_Jpsi_Pts_065080");
        Jpsi_Gen_Norm->SetName("nGen_Jpsi_Pts_065080");
      }
      if(iCat == 2){
        Eff_Jpsi->SetName("Eff_Jpsi_Pts_080100");
        Jpsi_Gen->SetName("Gen_Jpsi_Pts_080100");
        Jpsi_Reco->SetName("Reco_Jpsi_Pts_080100");
        hJpsi_Gen->SetName("hGen_Jpsi_Pts_080100");
        hJpsi_Reco->SetName("hReco_Jpsi_Pts_080100");
        Jpsi_Reco_Norm->SetName("nReco_Jpsi_Pts_080100");
        Jpsi_Gen_Norm->SetName("nGen_Jpsi_Pts_080100");
      }
      if(iCat == 3){
        Eff_Jpsi->SetName("Eff_Jpsi_Pts_100400");
        Jpsi_Gen->SetName("Gen_Jpsi_Pts_100400");
        Jpsi_Reco->SetName("Reco_Jpsi_Pts_100400");
        hJpsi_Gen->SetName("hGen_Jpsi_Pts_100400");
        hJpsi_Reco->SetName("hReco_Jpsi_Pts_100400");
        Jpsi_Reco_Norm->SetName("nReco_Jpsi_Pts_100400");
        Jpsi_Gen_Norm->SetName("nGen_Jpsi_Pts_100400");
      }
      Eff_Jpsi->Write();
      Jpsi_Gen->Write();
      Jpsi_Reco->Write();
      ...
      if(iCat == 0) sprintf(tmp1,"3.0 < p_{T} < 6.5 GeV/c");
      if(iCat == 1) sprintf(tmp1,"6.5 < p_{T} < 8.0 GeV/c");
      if(iCat == 2) sprintf(tmp1,"8.0 < p_{T} < 10.0 GeV/c");
      if(iCat == 3) sprintf(tmp1,"10.0 < p_{T} < 40.0 GeV/c");
      lt1->DrawLatex(0.21,0.91,tmp1);

      //Eff_Jpsi->Write();

      char tmp_pdf[512], tmp_png[512];
      if(iCat == 0) {sprintf(tmp_pdf,"plots/Eff_Jpsi_Pts_030065_dPhi_%s.pdf",cCd);sprintf(tmp_png,"plots/Eff_Jpsi_Pts_030065_dPhi_%s.png",cCd);}
      if(iCat == 1) {sprintf(tmp_pdf,"plots/Eff_Jpsi_Pts_065080_dPhi_%s.pdf",cCd);sprintf(tmp_png,"plots/Eff_Jpsi_Pts_065080_dPhi_%s.png",cCd);}
      if(iCat == 2) {sprintf(tmp_pdf,"plots/Eff_Jpsi_Pts_080100_dPhi_%s.pdf",cCd);sprintf(tmp_png,"plots/Eff_Jpsi_Pts_080100_dPhi_%s.png",cCd);}
      if(iCat == 3) {sprintf(tmp_pdf,"plots/Eff_Jpsi_Pts_100400_dPhi_%s.pdf",cCd);sprintf(tmp_png,"plots/Eff_Jpsi_Pts_100400_dPhi_%s.png",cCd);}
      gPad->Print(tmp_pdf);
      gPad->Print(tmp_png);
