...
void Plot_MCv2_Pts(){
  ...
  TFile *output = new TFile("MCCT_PrJpsi_v2_Pts_EffCor.root","RECREATE");
  ...
  for(int iCat = 0; iCat < 2; iCat++){
    for(int iSpec = 0; iSpec < 6; iSpec++){
      ...
      sprintf(tmp_input[0],"MCCT_PrJpsi_Pts_065080_dPhi_%s.root",cCd);
      sprintf(tmp_input[1],"MCCT_PrJpsi_Pts_080100_dPhi_%s.root",cCd);
      sprintf(tmp_input[2],"MCCT_PrJpsi_Pts_100400_dPhi_%s.root",cCd);
      ...
      TGraphErrors *g[10];
      if(bReco){
        g[0] = (TGraphErrors*) f1->Get("nReco_Jpsi_Pts_065080");
        g[1] = (TGraphErrors*) f2->Get("nReco_Jpsi_Pts_080100");
        g[2] = (TGraphErrors*) f3->Get("nReco_Jpsi_Pts_100400");
        //g[3] = (TGraphErrors*) f4->Get("nReco_Jpsi_Pts_130400");
      }else{
        g[0] = (TGraphErrors*) f1->Get("nGen_Jpsi_Pts_065080");
        g[1] = (TGraphErrors*) f2->Get("nGen_Jpsi_Pts_080100");
        g[2] = (TGraphErrors*) f3->Get("nGen_Jpsi_Pts_100400");
        //g[3] = (TGraphErrors*) f4->Get("nGen_Jpsi_Pts_130400");
      }
      ...
      for(int k = 0; k < 1; k++){
        ...
        lt1->DrawLatex(0.18, 0.75, "0.0 < |y| < 2.4");
      }

      //v_{2} = %.4f #pm %.4f (#chi^{2} / ndf = %.3f / %.0f)
      sprintf(tmp2,"6.5 < p_{T} < 8.0 v_{2}: %.4f #pm %.4f (#chi^{2} / ndf = %.3f / %.0f)", vTwo[0], vTwoErr[0], chis[0], ndf[0]);
      leg1->AddEntry(g[0],tmp2);
      sprintf(tmp2,"8.0 < p_{T} < 10.0 v_{2}: %.4f #pm %.4f (#chi^{2} / ndf = %.3f / %.0f)", vTwo[1], vTwoErr[1], chis[1], ndf[1]);
      leg1->AddEntry(g[1],tmp2);
      sprintf(tmp2,"10.0 < p_{T} < 40.0 v_{2}: %.4f #pm %.4f (#chi^{2} / ndf = %.3f / %.0f)", vTwo[2], vTwoErr[2], chis[2], ndf[2]);
      leg1->AddEntry(g[2],tmp2);
      //sprintf(tmp2,"13.0 < p_{T} < 40.0 v_{2}: %.4f #pm %.4f (#chi^{2} / ndf = %.3f / %.0f)", vTwo[3], vTwoErr[3], chis[3], ndf[3]);
      ...
      sprintf(tmp_png,"MCv2_%s_Pts_0405_%s.png", cRec, cCd);
      sprintf(tmp_pdf,"MCv2_%s_Pts_0405_%s.pdf", cRec, cCd);
      c1->SaveAs(tmp_png);
      c1->SaveAs(tmp_pdf);
      ...
      char tmpPts[512];
      sprintf(tmpPts,"gPts_%s_%s",cRec,cCd);
      TGraphErrors *gPts = new TGraphErrors(3, ptBins, vTwo, ptBinsErr, vTwoErr);
      gPts->SetName(tmpPts);
      output->cd();
      gPts->Write();

      double vTwoCorr[3] = {0.0};
      double vTwoCorrErr[3] = {0.0};
      for(int i = 0; i < 3; i++){
      ...
      sprintf(tmpPts,"gPts_%s_%s_ResCorr",cRec,cCd);
      TGraphErrors *gPtsCor = new TGraphErrors(3, ptBins, vTwoCorr, ptBinsErr, vTwoCorrErr);
      gPtsCor->SetName(tmpPts);
      output->cd();
      gPtsCor->Write();
    }
  }
