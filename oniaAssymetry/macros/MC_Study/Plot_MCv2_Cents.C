...
void Plot_MCv2_Cents(){
  //gROOT->ProcessLine(".x ../rootlogon.C");

  TFile *output = new TFile("MCCT_PrJpsi_v2_Cent_EffCor.root","RECREATE");

  double cBins[] = {355.4, 261.4178, 187.1470, 89.9};
  double cBinsErr[] = {0.0, 0.0, 0.0, 0.0};

  double res[4] = {0.6050,0.7841,0.7864,0.6419};
  double resErr[4] = {0.0028,0.0034,0.0041,0.0029};

  // iCat : gen, reco
  // iSpec : default/sailor/cowboy/etHFm/etHFp
  for(int iCat = 0; iCat < 2; iCat++){
    for(int iSpec = 0; iSpec < 3; iSpec++){
      ...
      sprintf(tmp_input[0],"MCCT_PrJpsi_Cent_0010_dPhi_%s.root",cCd);
      sprintf(tmp_input[1],"MCCT_PrJpsi_Cent_1020_dPhi_%s.root",cCd);
      sprintf(tmp_input[2],"MCCT_PrJpsi_Cent_2030_dPhi_%s.root",cCd);
      sprintf(tmp_input[3],"MCCT_PrJpsi_Cent_3060_dPhi_%s.root",cCd);
      ...
      TGraphErrors *g[10];
      if(bReco){
        g[0] = (TGraphErrors*) f1->Get("nReco_Jpsi_Cent_0010");
        g[1] = (TGraphErrors*) f2->Get("nReco_Jpsi_Cent_1020");
        g[2] = (TGraphErrors*) f3->Get("nReco_Jpsi_Cent_2030");
        g[3] = (TGraphErrors*) f4->Get("nReco_Jpsi_Cent_3060");
      }else{
        g[0] = (TGraphErrors*) f1->Get("nGen_Jpsi_Cent_0010");
        g[1] = (TGraphErrors*) f2->Get("nGen_Jpsi_Cent_1020");
        g[2] = (TGraphErrors*) f3->Get("nGen_Jpsi_Cent_2030");
        g[3] = (TGraphErrors*) f4->Get("nGen_Jpsi_Cent_3060");
      }
      ...
      hPad1->SetMaximum(1.5);
      hPad1->SetMinimum(0.4);
      ...
      for(int k = 0; k < 1; k++){
        ...
        lt1->DrawLatex(0.18, 0.81, "0.0 < |y| < 2.4");
        lt1->DrawLatex(0.18, 0.75, "6.5 < p_{T} < 40.0 GeV/c");
      }

      //v_{2} = %.4f #pm %.4f (#chi^{2} / ndf = %.3f / %.0f)
      sprintf(tmp2,"0 - 10 %% v_{2}: %.4f #pm %.4f (#chi^{2} / ndf = %.3f / %.0f)", vTwo[0], vTwoErr[0], chis[0], ndf[0]);
      leg1->AddEntry(g[0],tmp2);
      sprintf(tmp2,"10 - 20 %% v_{2}: %.4f #pm %.4f (#chi^{2} / ndf = %.3f / %.0f)", vTwo[1], vTwoErr[1], chis[1], ndf[1]);
      leg1->AddEntry(g[1],tmp2);
      sprintf(tmp2,"20 - 30 %% v_{2}: %.4f #pm %.4f (#chi^{2} / ndf = %.3f / %.0f)", vTwo[2], vTwoErr[2], chis[2], ndf[2]);
      leg1->AddEntry(g[2],tmp2);
      sprintf(tmp2,"30 - 60 %% v_{2}: %.4f #pm %.4f (#chi^{2} / ndf = %.3f / %.0f)", vTwo[3], vTwoErr[3], chis[3], ndf[3]);
      leg1->AddEntry(g[3],tmp2);
      leg1->Draw();
      lt1->SetTextSize(0.04);
      lt1->DrawLatex(0.645,0.88,"CMS Simulation");
      char tmp_ep[512], tmp_cd[512];
      if(bReco) lt1->DrawLatex(0.60,0.83,tmp_ep);
      ...
      sprintf(tmp_png,"MCv2_%s_Cents_0402_%s.png", cRec, cCd);
      sprintf(tmp_pdf,"MCv2_%s_Cents_0402_%s.pdf", cRec, cCd);
      c1->SaveAs(tmp_png);
      c1->SaveAs(tmp_pdf);
      ...
      char tmpCent[512];
      sprintf(tmpCent,"gCent_%s_%s",cRec,cCd);
      TGraphErrors *gCent = new TGraphErrors(4, cBins, vTwo, cBinsErr, vTwoErr);
      gCent->SetName(tmpCent);
      output->cd();
      gCent->Write();
      ...
      for(int i = 0; i < 4; i++){
        vTwoCorr[i] = (double) vTwo[i]/res[i]; 
        vTwoCorrErr[i] = (double) (sqrt((resErr[i]/res[i])*(resErr[i]/res[i])+(vTwoErr[i]/vTwo[i])*(vTwoErr[i]/vTwo[i])));
        vTwoCorrErr[i] = vTwoCorr[i] * vTwoCorrErr[i];
      }
      sprintf(tmpCent,"gCent_%s_%s_ResCorr",cRec,cCd);
      TGraphErrors *gCentCor = new TGraphErrors(4, cBins, vTwoCorr, cBinsErr, vTwoCorrErr);
      gCentCor->SetName(tmpCent);
      output->cd();
      gCentCor->Write();
    }
  }
...
void GetV2(TGraph *a, double *b, int c){
  ...
  TF1 *v2Fit2 = new TF1("v2Fit2","(2/TMath::Pi())*(1+2*[0]*TMath::Cos(2.0*x))",0,TMath::PiOver2());
  v2Fit2->SetLineWidth(1);
  v2Fit2->SetLineColor(c);
  v2Fit2->SetLineStyle(2);

  a->Fit(v2Fit2,"R");
  //a->Fit(v2Fit1,"qr");
  b[0] = v2Fit2->GetParameter(0);
  b[1] = v2Fit2->GetParError(0);
  b[2] = v2Fit2->GetChisquare();
  b[3] = v2Fit2->GetNDF();
  v2Fit2->Delete();
