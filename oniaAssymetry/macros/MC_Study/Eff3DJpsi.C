...
void Eff3DJpsi()   
{
  ...
  */
      ...
      for(int ict = 0; ict < nCentBins; ict++){
        for(int ipt = 0; ipt < nPtBins; ipt++){
          for(int irap = 0; irap < nRapBins; irap++){
            if(ict == (nCentBins-1)){
              if( (GenPosIn==1 && GenNegIn==1) && 
                  (GenJpsiRap >= rap_bound[irap]  && GenJpsiRap < rap_bound[irap+1] ) && 
                  (GenJpsiPt >= pt_bound[ipt] && GenJpsiPt < pt_bound[ipt+1]) &&
                  (gbin >= ct_bound[ict-4] && gbin < ct_bound[ict-1])) {
            ...
            for(int irap = 0; irap < nRapBins; irap++){
              if(ict == (nCentBins-1)){
                if((AllCut==1) && 
                    ...
                    (JpsiRap >= rap_bound[irap] && JpsiRap < rap_bound[irap+1]) &&  
                    (rbin >= ct_bound[ict-4] && rbin < ct_bound[ict-1])){
  ...
  TFile *outfile;
  char tmp_output[512];
  sprintf(tmp_output,"PrJpsi_%s.root", cCd);
  ...
  for(int ict = 0; ict < nCentBins; ict++){
    for(int ipt = 0; ipt < nPtBins; ipt++){
      for(int irap = 0; irap < nRapBins; irap++){
        ...
        if(ict == (nCentBins-1)){
          sprintf(gtmp,"hGenDiMuon_Cent_%1.f_%1.f_pt_%0.1f_%0.1f_rap_%0.1f_%0.1f",ct_bound[ict-4],ct_bound[ict-1],pt_bound[ipt],pt_bound[ipt+1],rap_bound[irap],rap_bound[irap+1]);
          sprintf(gtmp1,"plots/Gen/hGenDiMuon_Cent_%1.f_%1.f_pt_%0.1f_%0.1f_rap_%0.1f_%0.1f_%s.png",ct_bound[ict-4],ct_bound[ict-1],pt_bound[ipt],pt_bound[ipt+1],rap_bound[irap],rap_bound[irap+1],cCd);
          sprintf(rtmp,"hRecDiMuon_Cent_%1.f_%1.f_pt_%0.1f_%0.1f_rap_%0.1f_%0.1f",ct_bound[ict-4],ct_bound[ict-1],pt_bound[ipt],pt_bound[ipt+1],rap_bound[irap],rap_bound[irap+1]);
          sprintf(rtmp1,"plots/Rec/hRecDiMuon_Cent_%1.f_%1.f_pt_%0.1f_%0.1f_rap_%0.1f_%0.1f_%s.png",ct_bound[ict-4],ct_bound[ict-1],pt_bound[ipt],pt_bound[ipt+1],rap_bound[irap],rap_bound[irap+1],cCd);
        }else{
  ...
  for(int ict = 0; ict < nCentBins; ict++){
    for(int ipt = 0; ipt < nPtBins; ipt++){
      ...
      if(ict == (nCentBins-1)){
        sprintf(tmp_eff1D,"eff1D_Cent_%1.f_%1.f_Pt_%0.1f_%0.1f_%s",ct_bound[ict-4],ct_bound[ict-1],pt_bound[ipt],pt_bound[ipt+1],cCd); 
        sprintf(tmp_eff1D_png,"eff_1D_Cent_%1.f_%1.f_Pt_%0.1f_%0.1f_%s.png",ct_bound[ict-4],ct_bound[ict-1],pt_bound[ipt],pt_bound[ipt+1],cCd);
      }else{
  ...
  for(int ipt = 0; ipt < nPtBins; ipt++){
    for(int irap = 0; irap < nRapBins; irap++){
      cout<<" Eff ["<<ipt<<"]["<<irap<<"] - \t"<< eff[ict][ipt][irap][0] <<"\t"<< effErr[ict][ipt][irap][0] << "\t" << eff[ict][ipt][irap][1] <<"\t"
        << effErr[ict][ipt][irap][1] << "\t" << eff[ict][ipt][irap][2] <<"\t"<< effErr[ict][ipt][irap][2] << "\t" << eff[ict][ipt][irap][3] <<"\t" << effErr[ict][ipt][irap][3] << " " <<endl;
      dataFile<<" Eff ["<<ipt<<"]["<<irap<<"] - \t"<< eff[ict][ipt][irap][0] <<"\t"<< effErr[ict][ipt][irap][0] << "\t" << eff[ict][ipt][irap][1] <<"\t"
        << effErr[ict][ipt][irap][1] << "\t" << eff[ict][ipt][irap][2] <<"\t"<< effErr[ict][ipt][irap][2] << "\t" << eff[ict][ipt][irap][3] <<"\t" << effErr[ict][ipt][irap][3] << " " <<endl;
    }
  }
  */
