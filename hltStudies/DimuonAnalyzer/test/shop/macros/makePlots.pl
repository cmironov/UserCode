...
my $docentrality = 2; # 0 both, 1 minbias, 2 for central // or for different pp tracking
my $dohisto      = 0; # 0 both, 1 for efficiency, 2 for fake
my $dobody       = 0; # 0 for both, 1 for single, 2 for pair

my $domuon       = 1; # 0 for both, 1 global, 2 for sta
		...
		for(my $ih=$ihstart;$ih<$ihend;$ih++)
		  {
			...
			print "Running: $dimu,\t $type,\t $recomu ,\t $histo,\t $centrking,\t $cov \n, $inDir\n,$outDir \n,$body, \t $savefigs \n ";
			
			system("root -l -b -q 'plots.C+(\"$dimu\",\"$type\",\"$recomu\",\"$histo\",\"$centrking\",\"$cov\",\"$inDir\",\"$outDir\",$body,$savefigs)'");
		      }# single or di-muon
		  }#histogram type
	      }#type of reco muon
	  }#centrality
