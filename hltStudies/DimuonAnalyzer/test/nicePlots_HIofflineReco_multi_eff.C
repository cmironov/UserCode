...
void nicePlots_HIofflineReco_multi_eff()
{
  // takes output of DimuonAnalyzer and make eff plots (and other plots if wanted)
  // needs a ./figs/ directory
  //
  ...
  bool bSavePlots_indiv = false;
  bool is_allPlots =false;
  bool is_PtYMass = false;
  bool is_event_info = false;

  bool is_signal_only = false;
  //  bool is_signal_only = true;
  bool is_upsilon = false;
  bool is_jpsi = true;
  bool is_z = false;

  int nfiles =4;
  TGraphAsymmErrors* phPt_dimuEff[4];// dimension of nfiles at least
  TGraphAsymmErrors* phY_dimuEff[4];// dimension of nfiles at least
   
  if(!is_signal_only&&is_jpsi)
    {const char* inputFileName[] = {  "DimuonAnalyzer_Jpsi_embedding_500events_MassCut_barrel_GLB" , 
				      "DimuonAnalyzer_Jpsi_embedding_500events_MassCut_barrel_STA",
				      "DimuonAnalyzer_Jpsi_embedding_500events_MassCut_endcaps_GLB",
				      "DimuonAnalyzer_Jpsi_embedding_500events_MassCut_endcaps_STA"};
      const char* outputFileName ="DimuonAnalyzer_Jpsi_embedding_500events_MassCut";
      
    }
  
  if(is_signal_only&&is_jpsi)
   {  const char* inputFileName[] = {"DimuonAnalyzer_Jpsi_SignalOnly_1400events_MassCut_barrel_GLB",
				     "DimuonAnalyzer_Jpsi_SignalOnly_1400events_MassCut_barrel_STA",
				     "DimuonAnalyzer_Jpsi_SignalOnly_1400events_MassCut_endcaps_GLB",
				     "DimuonAnalyzer_Jpsi_SignalOnly_1400events_MassCut_endcaps_STA"};
     const char* outputFileName ="DimuonAnalyzer_Jpsi_SignalOnly_1400events_MassCut";
   }
 
 if(!is_signal_only&&is_upsilon)
   {  const char* inputFileName[] = {"DimuonAnalyzer_Upsilon_embedding_500events_MassCut_barrel_GLB",
				     "DimuonAnalyzer_Upsilon_embedding_500events_MassCut_barrel_STA",
				     "DimuonAnalyzer_Upsilon_embedding_500events_MassCut_endcaps_GLB",
				     "DimuonAnalyzer_Upsilon_embedding_500events_MassCut_endcaps_STA"};
     const char* outputFileName ="DimuonAnalyzer_Upsilon_embedding_500events_MassCut";
   }
 
 if(is_signal_only&&is_upsilon)
    {  const char* inputFileName[] = {"DimuonAnalyzer_Upsilon_SignalOnly_2000events_MassCut_barrel_GLB",
				     "DimuonAnalyzer_Upsilon_SignalOnly_2000events_MassCut_barrel_STA",
				     "DimuonAnalyzer_Upsilon_SignalOnly_2000events_MassCut_endcaps_GLB",
				     "DimuonAnalyzer_Upsilon_SignalOnly_2000events_MassCut_endcaps_STA"};
     const char* outputFileName ="DimuonAnalyzer_Upsilon_SignalOnly_2000events_MassCut";
   }
    if(!is_signal_only&&is_z)
      {const char* inputFileName[] = {"DimuonAnalyzer_Z_embedding_500events_MassCut_barrel_GLB",
				      "DimuonAnalyzer_Z_embedding_500events_MassCut_barrel_STA",
				      "DimuonAnalyzer_Z_embedding_500events_MassCut_endcaps_GLB",
				      "DimuonAnalyzer_Z_embedding_500events_MassCut_endcaps_STA"};
     const char* outputFileName ="DimuonAnalyzer_Z_embedding_500events_MassCut";
   }

    if(is_signal_only&&is_z)
   {  const char* inputFileName[] = {"DimuonAnalyzer_Z_SignalOnly_1000events_MassCut_barrel_GLB",
				     "DimuonAnalyzer_Z_SignalOnly_1000events_MassCut_barrel_STA",
				     "DimuonAnalyzer_Z_SignalOnly_1000events_MassCut_endcaps_GLB",
				     "DimuonAnalyzer_Z_SignalOnly_1000events_MassCut_endcaps_STA"};
     const char* outputFileName ="DimuonAnalyzer_Z_SignalOnly_1000events_MassCut";
   }
    const char* inputFileLocation = "root";
  ...
  if(is_signal_only&&is_upsilon) 
      ...
      else if(!is_signal_only&&is_z) 
	{
      ...
      else   if(is_signal_only&&is_jpsi) 
	{
	  int maxXMass2 = 5;
	  int maxXMass1 = 5;
	  int minXMass1 = 1;
  ...
  sprintf(szBuf,"%s_eff_pt",outputFileName);
  TCanvas *pc_pt = new TCanvas(szBuf,szBuf);
  sprintf(szBuf,"%s_eff_y",outputFileName);
  TCanvas *pc_y = new TCanvas(szBuf,szBuf);

  TLegend* leg = new TLegend(0.17,0.83,0.6,0.9);
  leg->SetFillColor(0);
  ...
  sprintf(szBuf,"%s/%s.root",inputFileLocation,inputFileName[ifile]);
  ...
  phPt_dimuEff[ifile] = new TGraphAsymmErrors();
  phPt_dimuEff[ifile]->BayesDivide(phPtReco,phPtGen,"");
  ...
  phY_dimuEff[ifile] = new TGraphAsymmErrors(); 
  phY_dimuEff[ifile]->BayesDivide(phYReco,phYGen,"");
 ...
 // set drawing axis
  ...
  phYAxis->SetMaximum(max_eff);
 
  TH1 *phPtAxis  = new TH1F("phPtAxis",";p_{T} [GeV/c];efficiency(reco/gen)",20,0.,20.);
  phPtAxis->SetDirectory(0);
  phPtAxis->SetMaximum(max_eff);
 
  // colors:
  int color = marker_color+ifile;
  phPt_dimuEff[ifile]->SetMarkerStyle(marker_style);
  phPt_dimuEff[ifile]->SetLineColor(marker_color-ifile); 
  phPt_dimuEff[ifile]->SetLineWidth(marker_size);

  phY_dimuEff[ifile]->SetMarkerStyle(marker_style);
  phY_dimuEff[ifile]->SetLineColor(marker_color-ifile); 
  phY_dimuEff[ifile]->SetLineWidth(marker_size);

  phPt_dimuEff[ifile]->SetMarkerColor(marker_color-ifile);//marker_color+ifile);
  phY_dimuEff[ifile]->SetMarkerColor(marker_color-ifile);

  pc_pt->cd();
  if(fit_eff)phY_dimuEff[ifile]->Fit("pol0","+","",20.,100.);
  if(ifile==0){
    phPtAxis->Draw();
    phPt_dimuEff[ifile]->Draw("P");
  }
  else    phPt_dimuEff[ifile]->Draw("P");
  
  pc_pt->Update();


    char label[512];
  sprintf( label, "%s",inputFileName[ifile] );
  // leg->SetHeader("The Legend Title");
  leg->AddEntry(phY_dimuEff[ifile],label,"l");  
  if(ifile==nfiles-1)leg->Draw();

    
  pc_y->cd();
  if(fit_eff)phY_dimuEff[ifile]->Fit("pol0","+","",-2.5,2.5);
  if(ifile==0){
    phYAxis->Draw();
    phY_dimuEff[ifile]->Draw("P");
  }
  else    phY_dimuEff[ifile]->Draw("P");
  pc_y->Update();
  if(ifile==nfiles-1)leg->Draw();
  ...
  // pt y and mass plots
  sprintf(szBuf,"%s_pt_y_mass",inputFileName[ifile]);
  ...
  if(is_allPlots){
  sprintf(szBuf,"%s_allplots",inputFileName[ifile]);
  ...
  if(is_event_info)
  {
    ...
    // canvas
    sprintf(szBuf,"%s_EventInfo",inputFileName[ifile]);
