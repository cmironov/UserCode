...
{  
  ...
  for(View<pat::Muon>::const_iterator it = muons->begin(), itend = muons->end(); it != itend; ++it){
	...
	if (myVertex.isValid()) {
	  ...
	  // change for run in 42x: AlgebraicVector3 vpperp(pperp.x(), pperp.y(), 0.);
	  ...
	  // this does'nt work in 421_X --pp@276
	  ...
	  double ctauErrPV = sqrt(vXYe.similarity(vpperp))*3.096916/(pperp.Perp2());
