...
muIsoExtractorCalo_(0),muIsoExtractorTrack_(0),muIsoExtractorJet_(0)
{
   ...
   minNHitsInnerTrack_      = iConfig.getParameter<int>("NHitsInnerTrack_Min");
...
bool MuonIdProducer::isGoodTrack( const reco::Track& track )
{
  ...
  if (track.pt() < minPt_ || (track.p() < minP_ && track.p() < minPCaloMuon_) || (track.numberOfValidHits() <minNHitsInnerTrack_)){ 
...
{
  if(muon.track()->pt() < minPt_ || muon.track()->p() < minP_ || muon.track()->numberOfValidHits() <minNHitsInnerTrack_) return false;
